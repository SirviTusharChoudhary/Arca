...
const Login = ({swap, onClose}) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    .catch((error) => {
        setError(error.code?.replace('auth/', '').replace(/-/g, ' ') || 'Sign in failed. Please try again.');
        setLoading(false);
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    .catch ((error) => {
        setError(error.code?.replace('auth/', '').replace(/-/g, ' ') || 'Login failed. Please check your credentials.');
        setLoading(false);
    });
  };
